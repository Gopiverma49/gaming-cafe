...
export const CustomerPortal: React.FC = () => {
  ...
  const myBookings = bookings.filter(
    (b) => b.customerName === user?.name || b.customerPhone === user?.phone
  );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-900/80 border border-slate-800 text-xs text-slate-300">
              ...
              <span>{stations.filter((s) => s.status === 'AVAILABLE').length} Available</span>
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-900/80 border border-slate-800 text-xs text-slate-300">
              <span className="w-2 h-2 rounded-full bg-cyan-400" />
              <span>{stations.filter((s) => s.status === 'OCCUPIED').length} In Session</span>
            </div>
          </div>
        </div>
